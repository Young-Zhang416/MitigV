...
DEFAULT_DISTURBANCE_PREFIX = (
    "You are a confused object detector to provide a fuzzy overview or impression of the image."
)


class ICDConfig(HFMitigatorConfig):
...
class ICD(HFMitigator):
    ...
    # -- input preparation -----------------------------------------------------
    def _prepare_inputs(self, images: Any, prompt: str, cfg: ICDConfig) -> dict[str, Any]:
    ...
    def _disturb_prompt(self, prompt: str, cfg: ICDConfig) -> str:
    ...
    def _step_logits(
        ...
        logits_std, past = self._forward(input_ids, attention_mask, inputs, past_key_values)
    ...
    def _expand_inputs_for_beams(self, inputs: dict[str, Any], num_beams: int) -> dict[str, Any]:
        ...
        if self._disturbed_attention_mask is not None:
            self._disturbed_attention_mask = self._disturbed_attention_mask.repeat_interleave(
                num_beams, dim=0
            )
        return expanded
    ...
    def _reorder_aux_cache(self, beam_idx: torch.Tensor) -> None:
        ...
        if self._disturbed_attention_mask is not None:
            self._disturbed_attention_mask = self._disturbed_attention_mask.index_select(
                0, beam_idx
            )
